...
step40 = Step(40, [step41], 118420, 19, [
              1020, 1140, 1290, 1460, 1670, 1920, 2240])
step39 = Step(39, [end], 36500, 20, [340, 380, 420, 480, 540, 620])
step38 = Step(38, [step43], 81900, 21, [
              600, 650, 730, 820, 920, 1040, 1190, 1380, 1600])
step37 = Step(37, [step43], 73880, 18, [820, 920])
step36 = Step(36, [step42], 238230, 17, [5520, 6250])
step35 = Step(35, [step43], 67220, 18, [
              420, 480, 540, 620, 720, 830, 980, 1190, 1440])
step34 = Step(34, [step42], 95000, 17, [620, 710, 810, 930, 1090])
step33 = Step(33, [step41], 132770, 18, [750, 850, 960, 1090])
step32 = Step(32, [step42], 61660, 19, [
              340, 360, 410, 460, 520, 580, 670, 760])
step31 = Step(31, [step39], 91250, 16, [750])
step30 = Step(30, [step37, step36], 85450, 22, [
              500, 550, 600, 670, 750, 850, 960, 1090, 1270])
step29 = Step(29, [step36, step37], 41660, 18, [
              690, 770, 880, 1000, 1150, 1350, 1590])
step28 = Step(28, [step36, step37], 112220, 18, [
              720, 810, 910, 1050, 1210, 1410, 1670, 2000, 2440])
step27 = Step(27, [step35, step34, step33], 103040,
              23, [590, 650, 720, 780, 880, 980, 1110])
step26 = Step(26, [step31], 71810, 22, [
              330, 360, 390, 440, 490, 550, 630, 710])
step25 = Step(25, [step32, step40], 61420, 19, [580, 630, 700, 780])
step24 = Step(24, [step31], 63330, 18, [780, 890, 1000,
              1140, 1320, 1540, 1810, 2190, 2660, 3340])
...
step19 = Step(19, [step25, step29], 134160, 23, [4530, 4940,
              5410, 5960, 5410, 5960, 6570, 7310, 8170, 9200, 10410])
step18 = Step(18, [step28, step24], 4830, 3, [1670])
step17 = Step(17, [step38], 270730, 41, [4270, 4480, 4730,
              4970, 5260, 5880, 6240, 6630, 7050, 7530, 8040, 8630])
step16 = Step(16, [step22], 37500, 15, [])
step15 = Step(15, [step23, step30, step20], 256570, 19,
              [7310, 8170, 9200, 10410, 11910, 13730])
...
step6 = Step(6, [step12], 35710, 21, [
             290, 310, 400, 390, 440, 500, 570, 660, 770])
...
def create_user(school_email, password, class_code, is_teacher):
    ...
    user = User(school_email=school_email, password=hashed_password,
                class_code=class_code, is_teacher=is_teacher)  # TODO add better is teacher logic
...
def progress():
    if request.method == "POST":
        ...
        if json["next"]:
            sim.next_day()
            if not sim.running and current_user.is_authenticated:
                save_sim()
                sim_doc = SimDoc.query.filter_by(
...
def show_results():
    if current_user.is_teacher:
        students = User.query.filter_by(
            is_teacher=False, class_code=current_user.class_code).all()
        names = []
        sims = []
...
def signup():
    ...
    if form.validate_on_submit():
        ...
        user = User(school_email=form.email.data, password=hashed_password,
                    class_code=form.class_code.data, is_teacher=False)  # TODO add better is teacher logic
        file_path = f'{os.path.join(app.config["SIM_FOLDER"])}{user.id}.pkl'

        db.session.add(user)
        db.session.commit()

        user_sim = SimDoc(doc=file_path, time=121,
                          cost=3820220, user_id=user.id)
        db.session.add(user_sim)
        db.session.commit()
...
def authorize():
    ...
    user = User.query.filter_by(
        school_email=token['userinfo']['email']).first()
    if user:
        user_login(user)
        ...
        if user_info['email'] == 'william.wood@students.dominican.edu':
            is_teacher = True
        create_user(user_info['email'],
                    token['id_token'], 'google', is_teacher)
        user = User.query.filter_by(
            school_email=token['userinfo']['email']).first()
        user_login(user)
        return redirect(url_for('main.render_sim'))
